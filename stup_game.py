...
#               ВАЖНО
# -------------------------------------------------------------------------------------------------------------
# По моему мнению задание далеко не уровня Lite, а уровня Ultra Pro
# честно: ниже код был частично взят из просторов интернета, но немного его изменил
# также пришлось детально самому разобрать весь текст внизу
# и изучить вещи, которые не давались в вебинаре.
# например, @property - предопределенная декорация, которая конвертирует функцию в атрибут (причем с ограничением = только чтение)
# даже для минимальной разработки нужно создавать отдельные классы: сама игра, игрок, действие,
# Пришлось также повозиться и разобраться с аннотацией к функции (ожидаемые возвращаемые параметры)
...
# достоинтсва карт
ACE = 'Т'
NOMINALS_CARDS = ['6', '7', '8', '9', '10', 'В', 'Д', 'К', ACE]
NOMINALS_SHORT = ['В', 'Д', 'К', ACE]

# поиск индекса по достоинству
NAME_TO_VALUE = {n: i for i, n in enumerate(NOMINALS_CARDS)}
...
DECK = [(nom, suit) for nom in NOMINALS_CARDS for suit in SUITS]
